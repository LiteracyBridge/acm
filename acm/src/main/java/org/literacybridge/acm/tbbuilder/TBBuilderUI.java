...
public class TBBuilderUI extends JFrame implements ActionListener {
  ...
  private void fillProjectList(JComboBox l) {
    ...
    // TODO: should be tied into ACMConfiguration.discoverDBs() -- but this
    // means refactoring ACM init to not be DB-specific at first
    if (dropbox.exists()) {
      File[] dirs = dropbox.listFiles(new FileFilter() {
        @Override
        public boolean accept(File path) {
          return path.isDirectory()
              && path.getName().startsWith(TBBuilder.ACM_PREFIX);
        }
      });
      for (File d : dirs) {
  ...
  private String[] getPackages(String project) {
    ...
    File packages = new File(dropbox,
        TBBuilder.ACM_PREFIX + project + "/TB-Loaders/packages");
  ...
  private String[] getLanguages(String project) {
    ...
    File packages = new File(dropbox,
        TBBuilder.ACM_PREFIX + project + "/TB-Loaders/TB_Options/languages");
    File[] dirs = packages.listFiles(new FileFilter() {
      @Override
      public boolean accept(File path) {
        return path.isDirectory();
  ...
  public TBBuilderUI() throws Exception {
    ...
    // TODO: allow for more than 2 images/packages; however we have never had a
    ...
    JComboBox groupAList = new JComboBox(); // should be a JList to allow
                                            // multiple groups in future

    languageImgBList = new JComboBox();
    JComboBox groupBList = new JComboBox(); // should be a JList to allow
                                            // multiple groups in future
            ...
            layout.createSequentialGroup()
                .addGroup(
                    layout.createParallelGroup(LEADING)
                        ...
                        .addComponent(
                            languageALabel)
                        .addComponent(groupALabel).addComponent(packageBLabel)
                        .addComponent(languageBLabel).addComponent(groupBLabel))
                .addGroup(layout.createParallelGroup(LEADING)
                    .addComponent(notice).addComponent(projectList)
                    .addComponent(updateYearList).addComponent(updateNumberList)
                    .addComponent(packageAList).addComponent(languageImgAList1)
                    .addComponent(groupAList).addComponent(packageBList)
                    .addComponent(languageImgBList).addComponent(groupBList)
                    .addComponent(buttonBuild).addComponent(buttonPublish)));
    layout.setVerticalGroup(layout.createSequentialGroup().addComponent(notice)
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(projectLabel).addComponent(projectList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(updateYearLabel).addComponent(updateYearList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(updateNumberLabel).addComponent(updateNumberList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(packageALabel).addComponent(packageAList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(languageALabel).addComponent(languageImgAList1))
        .addGroup(layout.createParallelGroup(BASELINE).addComponent(groupALabel)
            .addComponent(groupAList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(packageBLabel).addComponent(packageBList))
        .addGroup(layout.createParallelGroup(BASELINE)
            .addComponent(languageBLabel).addComponent(languageImgBList))
        .addGroup(layout.createParallelGroup(BASELINE).addComponent(groupBLabel)
            .addComponent(groupBList))
        .addGroup(
            layout.createParallelGroup(BASELINE).addComponent(buttonBuild))
        .addGroup(
            layout.createParallelGroup(BASELINE).addComponent(buttonPublish)));
    ...
    if (o instanceof JComboBox) {
      if (o == projectList) {
        if (projectList.getSelectedIndex() > 0) {
          ...
          String contentPackages[] = getPackages(
              projectList.getSelectedItem().toString());
          ...
          String contentLanguages[] = getLanguages(
              projectList.getSelectedItem().toString());
