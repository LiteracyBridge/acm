...
public class MoveStats {
    ...
    public static void main(String[] args) throws IOException {
        ...
        File[] subdirs = sourceDir.listFiles(new FilenameFilter() {
            ...
            public boolean accept(File dir, String name) {
                ...
                //       <project> is like uwr or CARE, one or more in the collected-data directory
                boolean good = false;
                // Get non-hidden directories only
                if (!name.startsWith(".")) {
                    File subdir = new File(dir, name);
                    if (subdir.isDirectory()) {
                        File collecteddata = FileNC(subdir, TBLoader.COLLECTED_DATA_SUBDIR_NAME);
                        if (collecteddata.exists()) {
                            // Check that every project directory has a talkingbookdata subdirectory.
                            File[] projects = collecteddata.listFiles();
                            boolean everyProjectHasTBData = false; // must be at least one project with TBData
                            for (File project:projects) {
                                ...
                                if (!tbdata.exists() || !tbdata.isDirectory()) {
                                    logger.warn(String.format("The 'project' directory %s has no expected %s subdirectory.", project.getName(), TALKING_BOOK_DATA));
                            ...
                            logger.warn(String.format("The 'laptop' directory %s has no expected %s subdirectory.", name, TBLoader.COLLECTED_DATA_SUBDIR_NAME));
                        }
                    }
                }
                return good;
        ...
        if (subdirs.length > 0) {
            ...
            for (File subdir:subdirs) {
                logger.info("Zipping " + subdir + " and moving to " + targetCollection.getAbsolutePath());
    ...
    /**
     * Copied from the lb-core-api project. There should be, obviously, only one, and this entire util-let should
     * probably not be in the ACM project. But, there are dependencies on the TBLoader that also need to be refactored
     * as part of splitting it out.
     * @param parent A File representing a directory that may contain a child file or directory.
     * @param child The name of a possible child file or directory, but not necessarily with the correct casing,
     *              for instance, TALKING_BOOK_DATA vs talkingbookdata.
