...
public class ControlAccess {
  ...
  private boolean setZipFilenames(String currentFilename) {
    ...
    if (currentFilename == null) {
      goodName = false;
      // no db zip exists in dropbox but the db has been checked in; no need for
      // setNextZipFilename, because time to shutdown
      ...
      String baseFilename = currentFilename.substring(
          DB_ZIP_FILENAME_PREFIX.length(), currentFilename.lastIndexOf('.'));
      try {
        int count = Integer.parseInt(baseFilename) + 1;
        nextFilename = new String(DB_ZIP_FILENAME_PREFIX + String.valueOf(count)
  ...
  public File getSandboxDirectory() {
    File fSandbox = null;
    if (isSandbox()) {
      fSandbox = new File(config.getTempACMsDirectory(),
          config.getSharedACMname() + "/" + Constants.RepositoryHomeDir);
    }
    return fSandbox;
  }

  private File getDBAccessFile() {
    return new File(config.getSharedACMDirectory(),
        Constants.DB_ACCESS_FILENAME);
  }
  ...
  private void deleteLocalDB() {
    try {
      ...
      // normally
      File oldDB = new File(config.getTempACMsDirectory(),
          config.getSharedACMname());
  ...
  public void init() {
    ...
    if (dbInfo.isCheckedOut()) {
      if (ACMConfiguration.getInstance().isForceSandbox()) {
        System.out.println(
            "Sandbox mode forced, but DB is currently checked out. Check DB in and restart with sandbox flag.");
        System.exit(0);
      }
      setSandbox(false);
      if (!ACMConfiguration.getInstance().isDisableUI()) {
        JOptionPane.showMessageDialog(null,
            "You have already checked out this ACM.\nYou can now continue making changes to it.");
      }
    } else {
      deleteLocalDB();
      determineRWStatus();
    ...
    config.setRepository(new CachingRepository(
        new FileSystemRepository(config.getCacheDirectory(),
            new FileSystemRepository.FileSystemGarbageCollector(
                config.getCacheSizeInBytes(), new FilenameFilter() {
                  @Override
                  public boolean accept(File file, String name) {
                    return name.toLowerCase()
                        .endsWith("." + AudioFormat.WAV.getFileExtension());
                  }
                })),
  ...
  private void deleteOldFiles(int numFilesToKeep) {
    List<File> files = Lists.newArrayList(
        config.getSharedACMDirectory().listFiles(new FilenameFilter() {
          @Override
  ...
  private boolean haveLatestDB() {
    ...
    File fileShouldHave = new File(config.getSharedACMDirectory(),
        filenameShouldHave);
    boolean status;
    if (fileShouldHave.exists()) {
      status = true;
    } else {
      // latest db zip to be checked in is not yet in dropbox, so find most
      // recent zip and force sandbox mode
      status = false;
  ...
  private void determineRWStatus() {
    ...
    if (!isOnline()) {
      ...
      if (setCurrentFileToLastModified()) {
        if (!ACMConfiguration.getInstance().isDisableUI()) {
          JOptionPane.showMessageDialog(null,
              "Cannot connect to Literacy Bridge server.");
        }
      } else {
        if (!ACMConfiguration.getInstance().isDisableUI()) {
          JOptionPane.showMessageDialog(null,
              "Cannot connect to Literacy Bridge server and no available database. Shutting down.");
        }
        System.exit(0);
      }
    } else {
      ...
      do {
        ...
        try {
          dbAvailable = checkOutDB(config.getSharedACMname(),
              new String("statusCheck"));
          break;
        } catch (IOException e) {
          dbAvailable = false;
          Object[] options = { "Try again", "Use Demo Mode" };
          if (!ACMConfiguration.getInstance().isDisableUI()) {
            onlineChoice = JOptionPane.showOptionDialog(null,
                "Cannot reach Literacy Bridge web server.  Do you want to get online now and try again or use Demo Mode?",
                "Cannot Connect to Server", JOptionPane.YES_NO_CANCEL_OPTION,
      ...
      if (!sandboxMode) {
        ...
        if (!ACMConfiguration.getInstance().isDisableUI()) {
          if (getCurrentZipFilename().equalsIgnoreCase(DB_DOES_NOT_EXIST)) {
            JOptionPane.showMessageDialog(null,
                "ACM does not exist yet. Creating a new ACM and giving you write access.");
          } else {
            Object[] options = { "Update Shared Database", "Use Demo Mode" };
            n = JOptionPane.showOptionDialog(null,
                "Do you want to update the shared database?",
                "Update or Demo Mode?", JOptionPane.YES_NO_OPTION,
                JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
          }
        }
...
//  }
    ...
    url = new URL("http://literacybridge.org/checkout.php?db=" + db + "&action="
        ...
        + "&version=" + Constants.ACM_VERSION + "&computername="
        + computerName);
    InputStream in;
    in = url.openStream();
  ...
  private boolean checkInDB(String db, String key, String filename)
      throws IOException {
  ...
  public boolean updateDB() {
    ...
    File outFile = new File(config.getSharedACMDirectory(),
        getNextZipFilename());
    int n;
    if (!ACMConfiguration.getInstance().isDisableUI()) {
      Object[] optionsSaveWork = { "Save Work",
          "Throw Away Your Latest Changes" };
      n = JOptionPane.showOptionDialog(null,
          "If you made a mistake you can throw away all your changes now.",
          "Save Work?", JOptionPane.YES_NO_CANCEL_OPTION,
          JOptionPane.QUESTION_MESSAGE, null, optionsSaveWork,
          optionsSaveWork[0]);
      if (n == 1) {
        n = JOptionPane.showOptionDialog(null,
            "Are you sure you want to throw away all your work since opening the ACM?",
    ...
    n = 0; // we always want to start this loop. Even if the user wants to
           // discard changes, we need to try to release the checkout
    while (!status && n == 0) {
      ...
      if (saveWork) {
        try {
          ...
          status = false;
          if (!ACMConfiguration.getInstance().isDisableUI()) {
            Object[] options = { "Keep Your Changes",
                "Throw Away Your Latest Changes" };
            n = JOptionPane.showOptionDialog(null,
                "There is a problem getting your changes into Dropbox.  Do you want to keep your changes and try to get this problem fixed or throw away your latest changes?",
                "Problem creating zip file on Dropbox",
          ...
          if (n == 1) {
            ...
            break; // keep changes - so don't try to check-in, which happens in
                   // the remainder of the while loop below
          }
        }
      }
      try {
        ...
        if (!status && saveWork
            && !ACMConfiguration.getInstance().isDisableUI()) {
          // Object[] options = {"Force your version","Throw Away Your Latest
          // Changes"};
          JOptionPane.showMessageDialog(null,
              "Someone has forced control of this ACM, so you cannot check-in your changes.\nIf you are worried about losing a lot of work, contact Cliff and he may be able to save you and your work.");
          saveWork = false; // zip is alredy written to dropbox an could be
                            // recovered. !saveWork & status will delete
                            // checkoutfile marker but not any zips.
          status = true; // to
          // if (n==0)
    ...
    if (!ACMConfiguration.getInstance().isDisableUI()) {
      if (status && saveWork)
        JOptionPane.showMessageDialog(null,
            "Your changes have been checked in.\n\nPlease stay online for a few minutes so your changes\ncan be uploaded (until Dropbox is 'Up to date').");
      else if (saveWork && !status)
        JOptionPane.showMessageDialog(null,
            "Your changes could not be checked in now, but you still have this ACM\nchecked out and can submit your changes later.");
      else if (status && !saveWork)
        JOptionPane.showMessageDialog(null,
            "Your changes have been discarded.");
      else if (!status && !saveWork)
        JOptionPane.showMessageDialog(null,
            "Could not release your checkout.  Please try again later so that others can checkout this ACM.");
    }

    if (status && saveWork) {
      // deleting old zip since we just got confirmation that the new zip was
      // checkedin
      deleteOldFiles(NUM_ZIP_FILES_TO_KEEP);
    }
    if (status) { // whether saving work or not, only delete checkout file if
                  // status==true, meaning checkIn was successful
      dbInfo.deleteCheckoutFile(); // do this first since it's most important to
                                   // be deleted and the next line sometimes is
                                   // unable to delete the entire directory
      // deleteLocalDB(); -- this line always fails to delete some files, so
      // we'll just delete on startup if there is no checkout file
    }
    return status;
  }
}
