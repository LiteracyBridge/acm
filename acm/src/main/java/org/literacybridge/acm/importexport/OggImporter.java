...
public class OggImporter extends AudioFileImporter {
    private Metadata metadata = null;
    private Set<Category> categories = null;

    OggImporter(File audioFile) {
        super(audioFile);
    }
    ...
    protected Metadata getMetadata() {
        if (metadata == null) {
            try {
                ...
                if (!loadedMetadata.containsField(DC_TITLE) && !isEmpty(oggMetadata.get("TITLE"))) {
                    loadedMetadata.put(DC_TITLE, oggMetadata.get("TITLE"));
                }
                // If no LB_PRIMARY_SPEAKER, use OGG artist.
                if (!loadedMetadata.containsField(LB_PRIMARY_SPEAKER) && !isEmpty(oggMetadata.get("ARTIST"))) {
                    loadedMetadata.put(LB_PRIMARY_SPEAKER, oggMetadata.get("ARTIST"));
                }
                // If no LB_DATE_RECORDED, use OGG date.
                if (!loadedMetadata.containsField(LB_DATE_RECORDED) && !isEmpty(oggMetadata.get("DATE"))) {
