...
public abstract class Committable {
  private static enum CommitState {
    ...
    ROLLBACK_FAILED // a rollback attempt failed - this object is now in an
                    // invalid state
  }
  ...
  private static enum DeleteState {
    ...
    DELETE_REQUESTED // a delete of this object was requested, but not committed
                     // yet
  }

  private static final Logger LOG = Logger
      .getLogger(Committable.class.getName());

  private CommitState commitState;
  private DeleteState deleteState;

  // remembers whether this is a new object that was added to the index, or
  // whether an existing object was updated
  ...
  public final void ensureIsCommittable() {
    if (deleteState == DeleteState.DELETED) {
      throw new IllegalStateException(
          "This object was deleted and must not be used anymore.");
    }

    if (commitState == CommitState.ROLLBACK_FAILED) {
      throw new IllegalStateException(
          "This object is in an undefined state due to a failed rollback attempt and must be discarded");
    }
  }
  ...
  public final void commit(Transaction t) {
    ...
    try {
      ...
      throw new RuntimeException(
          "IOException while committing Committable: " + this.toString(), e);
    } finally {
      if (!success) {
  ...
  /**
   * Returns true, if this commit added a new object to the index, and false, if
   * an existing object was updated.
   */
  public abstract boolean doCommit(Transaction t) throws IOException;

  /**
   * Resets the internal state of this object to the version stored in the
   * index.
   */
  public final void rollback(Transaction t) {
    ...
    try {
      ...
      throw new RuntimeException(
          "IOException while rolling back Committable: " + this.toString(), e);
    } finally {
      if (!success) {
