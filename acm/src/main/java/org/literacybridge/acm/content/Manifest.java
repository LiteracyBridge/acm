...
public class Manifest implements Persistable {
	...
	List<ReferencedFile> getReferencedFiles() {
		...
		for (PersistentReferencedFile file : mManifest
				.getPersistentReferencedFileList()) {
			referencedFiles.add(new ReferencedFile(file));
		}
		return referencedFiles;
	}

	public void addReferencedFile(ReferencedFile file) {
		mManifest.addPersistentReferencedFile(file.mReferencedFile);
	}

	// TODO: if we want per-file revisions, then we need to add a revision to
	// this class
	public static class ReferencedFile implements Persistable {
		...
		// actual file (e.g. audio track) that his ReferencedFile instance
		// points to
		File file;

		// fallback file, e.g. there could be a mp3 file and an additional wav
		// fallback file
		...
		public String getLocation() {
			return mReferencedFile.getLocation();
		}
		
		public void setLocation(String location) {
			mReferencedFile.setLocation(location);
		}
		
		public ReferencedFile commit() {
			mReferencedFile = mReferencedFile
					.<PersistentReferencedFile> commit();
			return this;
		}
		...
		public ReferencedFile refresh() {
			mReferencedFile = mReferencedFile
					.<PersistentReferencedFile> refresh();
			return this;
		}
	}
}
