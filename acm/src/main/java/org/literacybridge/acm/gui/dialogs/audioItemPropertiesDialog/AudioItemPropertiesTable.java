...
public class AudioItemPropertiesTable extends JXTable {
  ...
  public AudioItemPropertiesTable(final AudioItemPropertiesDialog dialog) {
    setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
      ...
      public Component getTableCellRendererComponent(JTable table, Object value,
        ...
        if (AudioItemPropertiesModel.EDIT_COL == column
        ...
        if (e.getClickCount() == 2) {
          ...
          if (value != null) {
            ...
            if (!StringUtils.isEmpty(text)) {
              ...
              AudioItemProperty<?> property = getAudioItemPropertiesModel()
                  .getAudioItemProperty(row);
              if (property instanceof AudioItemProperty.MetadataProperty) {
                if (((AudioItemProperty.MetadataProperty) property)
                    .getMetadataField() == MetadataSpecification.DC_RELATION) {
                  searchRequest(text);
  ...
  public TableCellEditor getCellEditor(int row, int column) {
    ...
    if (property instanceof AudioItemProperty.MetadataProperty) {
      if (((AudioItemProperty.MetadataProperty) property)
          .getMetadataField() == MetadataSpecification.LB_MESSAGE_FORMAT) {
        return new DefaultCellEditor(messageFormatBox);
      }
      if (((AudioItemProperty.MetadataProperty) property)
          .getMetadataField() == MetadataSpecification.LB_TARGET_AUDIENCE) {
