...
public class SimpleSoundPlayer extends Observable
    implements ISoundPlayer, ControllerListener, Runnable {
  ...
  private void stop(boolean reposition) {
    if (player != null) {
      ...
      if (!reposition) {
        playFromSec = player.getMediaTime().getSeconds(); // save current player
                                                          // position
      }
      // Update listeners
      playThread = null;
      playerState = PlayerState.PAUSED;
      updatePlayerListeners(); // call explicit as player thread already
                               // interrupt
    }
  }
