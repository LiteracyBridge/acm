...
public class AudioItemImportView extends Container implements Observer {
  ...
  private void createTable() {
    AudioItemImportModel
        .initializeTableColumns(getColumnTitles(LanguageUtil.getUILanguage()));
    ...
    table.addHighlighter(HighlighterFactory.createAlternateStriping(Color.white,
  ...
  private String[] getColumnTitles(Locale locale) {
    ...
    columnTitleArray[AudioItemImportModel.TITLE] = LabelProvider
        .getLabel(LabelProvider.AUDIO_ITEM_TABLE_COLUMN_TITLE, locale);
    columnTitleArray[AudioItemImportModel.CATEGORIES] = LabelProvider
        .getLabel(LabelProvider.AUDIO_ITEM_TABLE_COLUMN_CATEGORIES, locale);
    columnTitleArray[AudioItemImportModel.LANGUAGES] = LabelProvider
        .getLabel(LabelProvider.AUDIO_ITEM_TABLE_COLUMN_LANGUAGE, locale);
  ...
  private void initColumnSize() {
    ...
    table.getTableHeader().getColumnModel()
        .getColumn(AudioItemImportModel.INFO_ICON).setMaxWidth(25);
    table.getTableHeader().getColumnModel()
        .getColumn(AudioItemImportModel.TITLE).setPreferredWidth(250);
    table.getTableHeader().getColumnModel()
        .getColumn(AudioItemImportModel.CATEGORIES).setPreferredWidth(150);
    table.getTableHeader().getColumnModel()
        .getColumn(AudioItemImportModel.LANGUAGES).setPreferredWidth(25);
  }
